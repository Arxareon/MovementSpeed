...
local presets = {
	...
	[1] = {
		name = ns.strings.options.speedDisplay.quick.presets.list[0], --Under Minimap Clock
		data = {
			position = {
				anchor = "TOPRIGHT",
				offset = { x = -69, y = -178 },
...
local UpdatePlayerSpeed = select(4, GetBuildInfo()) < 30000 and function() playerSpeed =  GetUnitSpeed("player") end or function()
...
local function SetDisplaySize(display, height, valueType, decimals)
	...
	local ratio = 3.1 + ((decimals or db.speedDisplay.value.decimals) > 0 and 0.25 + (decimals or db.speedDisplay.value.decimals) * 0.58 or 0)
	if (valueType or db.speedDisplay.value.type) == 1 then ratio = ratio + 0.3
	elseif (valueType or db.speedDisplay.value.type) == 2 then
		ratio = ratio + 3.1 + ((decimals or db.speedDisplay.value.decimals) > 0 and 0.25 + (decimals or db.speedDisplay.value.decimals) * 0.58 or 0)
	end
...
local function StartPlayerSpeedUpdates()
	if db.playerSpeed.throttle then
		...
		frames.playerSpeed.updater.ticker = C_Timer.NewTicker(db.playerSpeed.frequency, function()
			...
			wt.SetVisibility(frames.playerSpeed.display, not db.speedDisplay.visibility.autoHide or playerSpeed ~= 0)
		end)
	else
		--Set an update event
...
local function CreateValueOptions(parentFrame)
	...
	frames.options.speedDisplays.value.type = wt.CreateSelector({
		...
		optionsData = {
			...
			storageKey = "type",
			onChange = { UpdateDisplaySizes = function() SetDisplaySize("playerSpeed") end, }
		}
	})
	--Slider: Decimals
...
local function CreateFontOptions(parentFrame)
	...
	frames.options.speedDisplays.font.family = wt.CreateDropdown({
		...
		optionsData = {
			...
			onChange = {
				UpdateDisplayFonts = function() frames.playerSpeed.text:SetFont(db.speedDisplay.font.family, db.speedDisplay.font.size, "THINOUTLINE") end,
	...
	frames.options.speedDisplays.font.color = wt.CreateColorPicker({
		...
		optionsData = {
			...
			workingTable = db.speedDisplay.font,
			storageKey = "color",
			onChange = { UpdateDisplayFontColors = function() frames.playerSpeed.text:SetTextColor(wt.UnpackColor(db.speedDisplay.font.color)) end, }
		}
	})
end
...
local function CreateBackupOptions(parentFrame)
	...
	frames.options.advanced.backup.string, backupBox = wt.CreateEditScrollBox({
		...
		size = { width = parentFrame:GetWidth() - 35, height = 276 },
