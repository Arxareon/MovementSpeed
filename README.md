# <img src="https://cdn.wago.io/thumbnails/h0X1zDHgMB6xDOFicjSfOsrq6JkMXQS2K3x9KYI3.png" alt="Movement Speed Logo" width="40" height="40" align="top"> Movement Speed

[![GitHub Sponsor](https://img.shields.io/static/v1?label=GitHub&nbsp;Sponsor&message=%E2%9D%A4&color=EA4AAA&style=for-the-badge&logo=GitHub)](https://github.com/sponsors/Arxareon)
[![PayPal Donation](https://img.shields.io/static/v1?label=PayPal&nbsp;Donation&message=%E2%9D%A4&&color=0F9CD8&style=for-the-badge&logo=data:image/svg+xml;base64,)](https://www.paypal.com/donate/?hosted_button_id=Z4FSAFKA5LX98)

*Movement Speed shows your character's current speed in a movable and customizable display, or anyone else's in the inspect tooltip with fast updates.*

> **Important notice:** Movement Speed was built with [Widget Tools](https://github.com/Arxareon/WidgetTools), and it requires this dependency to be installed. It is not bundled in.

### Features
- See your own movement speed (in percentage and/or yards per second) within a customizable and repositionable text display.
- View the movement speed of other players or NPCs as well integrated into the inspect tooltip that appears when mousing over them.
- Set up Movement Speed in the Interface Options, access select options via the **/movespeed** chat command.
- Includes multi-version support for the modern game and all versions of Classic!

### Support

Alongside the one-time & monthly donation options featured above, watching ads on [**CurseForge**](https://www.curseforge.com/wow/addons/movement-speed) or [**Wago**](https://addons.wago.io/addons/movement-speed) also supports my work. You can keep up to date via the CurseForge or Wago App *as well as **[WoWUP](https://wowup.io/)** if you enable CurseForge or Wago.io as an addon provider*.

#### *Thank you for your continued support,*
Celierra

### Other Addons

| Icon | Addon | Description | Links |
|-|-|-|:-:|
| <img src="https://cdn.wago.io/thumbnails/JmYhfPLmyAFRdMQE7iiyCM5lFom6kixGAZCRoT14.png" alt="Remaining XP Logo" width="20" height="20" align="top"> | **Remaining XP** | Shows the XP amount needed to level up, tracks Rested XP gain, offers a customizable XP bar & much more. | [GitHub](https://github.com/Arxareon/RemainingXP) • [CurseForge](https://www.curseforge.com/wow/addons/remaining-xp) • [Wago](https://addons.wago.io/addons/remaining-xp) |
| <img src="https://cdn.wago.io/thumbnails/5MMvxRkfl5IEs0Ln0ChGHpn79qJJ7m7MimNvYFX3.png" alt="Widget Tools Logo" width="20" height="20" align="top"> | **Widget Tools** | This addon is providing a development toolset as a foundation for other addons mainly focused on utilities, settings and UI widget management. | [GitHub](https://github.com/Arxareon/WidgetTools) • [CurseForge](https://www.curseforge.com/wow/addons/widget-tools) • [Wago](https://addons.wago.io/addons/widget-tools) |

### License
All Rights Reserved.

- - -
Thank you for checking out Movement Speed!
If you have any comments, wishes or problems, please, don't be afraid to share them. GLHF!

*Arxareon*
