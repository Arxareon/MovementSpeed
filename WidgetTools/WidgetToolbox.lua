...
if not next(ns.WidgetToolbox) then
	...
	--- - **title**? string *optional* — Text to be shown as the title of the options panel | ***Default:*** [C_AddOns.GetAddOnMetadata(**t.addon**, "title")](https://wowpedia.fandom.com/wiki/API_GetAddOnMetadata)
	...
	wt.CreateOptionsCategory = function(t)
		...
		local title = t.title or wt.Clear(C_AddOns.GetAddOnMetadata(t.addon, "title")):gsub("^%s*(.-)%s*$", "%1")
		...
		if wt.classic then
			...
			local defaultWarning = wt.CreatePopup({
				...
				text = strings.options.warning:gsub("#TITLE", wt.Clear(C_AddOns.GetAddOnMetadata(t.addon, "title")) .. (t.parent and (": " .. title) or "")),
